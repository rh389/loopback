...
module.exports = function(registry) {
  ...
  PersistedModel.subscribe = function(options, cb) {
    ...
    var Model = this;
    var buffer = [];
    var changes = new ReadableStream({objectMode: true});

    changes._read = function() {
      var change = buffer.shift();
      if (change) {
        changes.push(change);
      }
    };
    ...
    Model.observe('after delete', afterDelete);
    ...
    function afterSave(ctx, next) {
      ...
      if (ctx.isNewInstance) {
        ...
        change.type = 'update';
        change.where = ctx.where;
      }

      change.data = ctx.data || ctx.instance;
